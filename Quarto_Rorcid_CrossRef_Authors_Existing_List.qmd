...
# [Rorcid_Crossref_Authors_Existing_List]{.underline}
...
![](images/clipboard-718373545.png)

\^This table was made in excel ( I did not figure our how to make a proper looking table with the build in quarto table feature. It could also be made in a separate site/html file and a link could be added or it could be called for to show the data set on a separate file)

----------

ANOTHER METHOD (But felt too long. Couldbe better for going into more details and again just added as a weblink or to the very bottom as a glossary)

Your variables (variables specifically connected to the user):
...
**1.** Defining your employment variable

```{r} myEmployment <- rorcid::orcidEmployments(myOrcidsData$orcidIdentifierPath)}
```

**2.** WRITE/READ JSON uncomment to work with this data outside of R or read it back in later
...
reads it back in, if necessary

```{r}
myEmployment <- read_json("./data/processed/employment.json", simplifyVector = TRUE)
```

**3.** Extract the employment data and mutate the dates

```{r} myEmploymentData <- myEmployment %>%   purrr::map(., purrr::pluck, "affiliation-group", "summaries") %>%    purrr::flatten_dfr() %>%   janitor::clean_names() %>%   dplyr::mutate(employmentSummaryEndDate = anytime::anydate(employmentSummaryEndDate/1000),                 employmentSummaryCreatedDateValue = anytime::anydate(employmentSummaryCreatedDateValue/1000),                 employmentSummaryLastModifiedDateValue = anytime::anydate(employmentSummaryLastModifiedDateValue/1000))}
```
...
**5.** Views the unique institutions in the organizations names columns
...
Here you can view the variation in organization names by looking at my_organization_filtered *(This will open in a new tab)*

```{r}
view(myOrganizations)
```

**Note:** this will give you employment records only

-    Each row represents a single employment record for an individual

-   The name_value variable refers specifically to the name of the person or system that wrote the record

    -   NOT the "name of the individual"

## ------------------------------------ Individuals name ----------------------------------------

To get the "name of the individual", you must first get all the unique ORCID IDs from the dataset
...
**1.** We must strip out the ORCID ID from the 'path' variable first and put it in it's own value and use it

-   We do this using str_sub from the stringr package

-   We can also select and reorder the columns we want to keep (As shown below)
...
**2.** We create a new vector uniqueOrcids that includes only unique ORCID IDs from our filtered dataset
...
**3.** then run the following expression to get all biographical information for those iDs.

##### IMPORTANT: This may take anywhere from a few seconds to a few minutes (e.g. for Temple University's data \[\~700 IDs\], this took \~1.5 minutes)

```{r}
myOrcidPerson <- rorcid::orcid_person(uniqueOrcids)
```

**4.** We construct a data frame from the response.
...
**5.** Join it back with the employment records so that the employment data now includes organization city, region, country
...
**1.** create a vector of unique, unduplicated ORCID IDs from that file
...
**2.** Call the orcidWorks function to collect all works associated with each ID
...
**3.** WRITE/READ JSON to work with this data outside of R or read it back in later
...
**4.** Turn the JSON file into a unique data frame by:

1.   looping through the file
2.  extracting ("pluck") the object
3.  bind the rows together with(this is the "\_dfr" part of map_dfr)
4.  clean column names
5.  convert the dates from Unix time to yyyy-mm-dd
...
**5.** We only want to keep works that have an external identifier (specifically, a DOI)

How?

1.  filter to keep only objects that have an external_id value
2.  unnest/expand those to include a row for every work + external id value
...
**6.** From those unnested external IDs, we want to keep only those with a DOI

-   Reason: that is the value we'll use to look up the items in Crossref.

-   We will then select relevant columns + create a new column DOI that the external_id_valu
...
dois <- myWorksExternalIDs %>%
  ...
  select(type, path, title_title_value, external_id_type, external_id_value, external_id_relationship,
         ...
         journal_title_value) %>%
  #creating a new column DOI that takes the external_id_value column
  mutate(doi = tolower(external_id_value),
         orcidIdentifier = str_sub(path, 2, 20))
```

there are some duplicated values here: we can't just look at duplicate DOIs because some of these works were co-authored, and we want to keep that data (i.e. unique orcid + doi combinations)

Other? -------------------------------------------

This function will let you look at observations where both the orcid ID and the DOI are duplicated in case you want to review them more closely.

In our case below, we just keep the first appearance of a unique orcid + doi combination and discardall subsequent ones.
...
**7.** Here we are preparing the orcid dataset for merging to publications.
...
**8.** Finally, we remove the duplicates by creating a new variable that is a combination of the orcid ID and the DOI, and keeping only the first instance.

We then join that to our cleaned orcid ID file and write to csv
...
**1.** Start by subsetting our unduped dois to include only since the year that we want
...
**2.** Loop through the column of dois and perform a function that prints the doi (this allows you to ensure it's progressing)

-   there will be warning messages for any DOIs not found at CrossRef

##### IMPORTANT: This will take a long time for large datasets (e.g. for Temple University's 2022 data \[800+ DOIs\], this took \~6 minutes)
...
**3.** WRITE/READ JSON uncomment to work with this data outside of R or read it back in later
...
2.  Extract ("pluck" function) the object called "data"
3.  Bind it together into a dataframe (the "dfr" part of map_dfr)
4.  Clean the names up and filter to remove any duplicates
...
**5.** Next we want to prepare our orcid data frame to merge to the crossref data by selecting only the relevant columns.
...
This will help us fill in blanks later if we start building a dataframe of full author names with their ORCID

***Start with the orcidPersonEmploymentJoin dataframe of employment data for home authors***

**1.** Create a fullname identifier for the home author that is stripped of punctuation and whitespace
...
**3.** Get the credit_name, an alternate version of their name and make a row for that
...
**4.** Strip the fullname identifier of punctuation and whitespace
...
**6.** Concatenate these two data frames to start our author ORCID ID reference table
...
These variables may not be required/ I could not find the necessary use.
